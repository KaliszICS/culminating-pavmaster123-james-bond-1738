...
public class Player extends Thing{
    ...
    public void update(double timeX, double timeY){
        ...
        if(timeY == 1){ // if the player is to collide on the x-axis, make sure the player doesnt get stuck in the block and can move after
            if(this.goingRight){
    ...
    /**
     * Checks if the Player is about to collide with a given Thing.
     * @param other The thing to check a possible collision with
     * @return A CollisionUpdate detailing which axis must be updated to avoid the collision, if any at all.
     */
